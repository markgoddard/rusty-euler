...
fn problem13() {
    ...
    let large_ones: [Vec<char>; 100] = large_ones.map(str::chars).map(Iterator::collect);
    let mut total = Vec::<char>::new();
    let mut sum = 0;
    for i in (0..large_ones[0].len()).rev() {
        for j in 0..large_ones.len() {
            sum += large_ones[j][i].to_digit(10).unwrap();
        }
        let digit = sum % 10;
        total.push(std::char::from_digit(digit, 10).unwrap());
        sum /= 10
    }
    loop {
        if sum == 0 {
    ...
    let result: String = total.iter().rev().take(10).collect();
    println!("Problem 13 {result}")
}
